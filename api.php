<?php
  ...
  if(empty($_GET['do'])) header('location:./');
      ...
      case 'slide':
        ...
        foreach($query->fetchAll() as $row){
          if($row['display'] != 0){
            ...
            if($row['btn'] != ''){
              $btn = '          
              <a href="'.$row['btnA'].'" class="btn btn-primary">'.$row['btn'].'</a>
              ';
            }
            $liData .= '
              <li data-target="#carouselExampleCaptions" data-slide-to="'.$num.'" ></li>
            ';
            $data .= '
              <div class="carousel-item  h-100 ">
                <img src="img/slide/'.$row['img'].'" class="d-block" alt="'.$row['title'].'">
                <div class="carousel-caption d-none d-md-block" style="top: 35%;">
                  <h1 class=" mt-5">'.$row['title'].'</h1>
                  <p>'.$row['info'].'</p>
                  '.$btn.'
                </div>
              </div>
        ...
        if(empty($_POST['list'])){
          ...
          function cartArea($id, $table, $num, $size, $color){
            ...
            foreach($sql->fetchAll() as $row){
              ...
              '
                <tr>
                  <td><a href="" class="d-block"><img src="../img/'.$table.'/'.$row['img1'].'"  alt="'.$row['title'].'"></a></td>
                  <td>
                    <p>'.$row['title'].'</p>
